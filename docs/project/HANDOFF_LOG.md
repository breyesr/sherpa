...
- **2026-06-29 (Epic 141 - B2C Product & Category Catalog Activation & UI Gating Backlog)**: Defined and added Epic 141 to `docs/project/BACKLOG.md` to reuse the shared `Product` and `Category` database models for B2C (BASIC) accounts, while dynamically gating B2B-specific fields (such as wholesale quantity thresholds and brands) in both the catalog view and edit forms when BASIC vertical is active. Relocated Category and Products as flat top-level sibling items in the B2C sidebar menu.
