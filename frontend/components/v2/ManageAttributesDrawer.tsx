...
export default function ManageAttributesDrawer({ isOpen, onClose, business, token }: ManageAttributesDrawerProps) {
  ...
  useEffect(() => {
    if (isOpen && business) {
      ...
      const attributes = featuresConfig?.services?.attributes || [];
  ...
  const handleSave = async () => {
    ...
    setError('');
    try {
      const currentFeaturesConfig = (business.features_config as any) || {};
      const newFeaturesConfig = {
        ...currentFeaturesConfig,
        services: {
          ...(currentFeaturesConfig.services || {}),
          attributes: fields
        }
      };
  ...
  return (
    <Drawer 
      isOpen={isOpen} 
      onClose={onClose} 
      title="Manage Service Attributes" 
      size="standard"
      footer={footerContent}
    >
      <div className="space-y-6">
        <p className="text-sm text-gray-500">
          Update the display labels of your custom service attributes or remove attributes you no longer need.
        </p>
